...
def parse_arguments(console: Console) -> Download | Logout:
    ...
    _configure_logging(console, debug_mode=debug)
    ...
    if logout:
        ...
        if save_dir:
            parser.error("--logout cannot be used with --save-dir")
        return Logout()

    if url is None:
        parser.error("A URL is required unless --logout is specified")
    if save_dir and directory is None:
        parser.error("--save-dir requires a directory")

    resolved = _resolve_directory(directory, save_dir=save_dir)
    logger.debug('Determined output directory: "%s"', resolved)

    return Download(url=url, directory=resolved)
